...
"""
    - Đưa AGV di chuyển theo lộ trình.
"""
...
class goalControl():
    def __init__(self):
        ...
        rospy.init_node('goal_control_fake', anonymous=True)
        ...
        self.vel_x_min = rospy.get_param('~vel_x_min')
        self.vel_theta_max = rospy.get_param('~vel_theta_max')
        self.vel_theta_min = rospy.get_param('~vel_theta_min')
        self.tolerance_xy_max = rospy.get_param('~tolerance_xy_max')
        self.tolerance_xy_min = rospy.get_param('~tolerance_xy_min')
        self.tolerance_theta_max = rospy.get_param('~tolerance_theta_max')
        self.tolerance_theta_min = rospy.get_param('~tolerance_theta_min')

        self.tolerance_theta = rospy.get_param('~tolerance_theta')
        self.tolerance_rot_step1 = rospy.get_param('~tolerance_rot_step1')

        self.vel_rot_step1 = rospy.get_param('~vel_rot_step1')
        self.vel_rot_step_f = rospy.get_param('~vel_rot_step_f')

        self.gioihan_lui  = rospy.get_param('~gioihan_lui')
        self.K_x = rospy.get_param('~he_so')
        self.dist_ahead_max = rospy.get_param('~khoang_nhin_truoc_max')
        self.dist_ahead_min = rospy.get_param('~khoang_nhin_truoc_min')
        self.khoang_offset = rospy.get_param('~khoang_offset')
        self.needRotatyFinish = rospy.get_param('~need_rotaty_finish')

        self.rate = rospy.Rate(self.rate)

        rospy.Subscriber("/%s/request_move" % self.agv_name, Move_request, self.move_callback)
        ...
        rospy.Subscriber("/%s/HC_info" % self.agv_name, HC_info, self.zone_callback)
        self.zone_lidar = HC_info()
        self.is_check_zone = False	

        rospy.Subscriber("/%s/POWER_info" % self.agv_name, POWER_info, self.MAIN_callback) 
        ...
        # rospy.Subscriber('/%s/safety_NAV' % self.agv_name, Int8, self.cbZoneNAV, queue_size = 1)
        # self.is_ZoneNav = False
        # self.dataZoneNav = Int8()

        self.pub_respond = rospy.Publisher("/%s/respond_move" % self.agv_name, Move_respond, queue_size= 20)
        self.pub_move = Move_respond()

        self.pub_requestFields = rospy.Publisher("/HC_fieldRequest", Int8, queue_size= 20)
        self.oldselectfield = 0

        self.pub_stt_goal = rospy.Publisher('/%s/status_goal_control' % self.agv_name, Status_goal_control, queue_size=10) 
        self.pub_path_local = rospy.Publisher('/%s/path_plan_local' % self.agv_name, Path, queue_size= 20)
